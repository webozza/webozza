		...
		var Plugin = {
			...
			dataRender: function(action) {
				...
				var buildMeta = function() {
					...
					if (meta.limit === 0) {
						meta.limit = options.data.pageSize || 10;
					}
					meta.total = datatable.dataSet.length;
					var start = Math.max(meta.limit * (meta.page_no - 1), 0);
					var end = Math.min(start + meta.limit, meta.total);
				...
				var afterGetData = function(result) {
					...
					if (options.pagination) {
						if (options.data.serverPaging && options.data.type !== 'local') {
							// server pagination
                            var serverMeta = Plugin.getObject('result', result || null);
                            // console.log(serverMeta)
							if (serverMeta !== null) {
                                let metaData = {
                                    page_no: serverMeta.page_no || 1,
                                    limit: serverMeta.limit || 10,
                                    total: serverMeta.total || 0
                                }
                                // console.log(metaData)
								Plugin.paging(metaData);
							} else {
			...
			getData: function() {
				...
				if (options.data.type === 'remote') {
                    ...
                    // // Mostafiz Version start
                    // console.log(data);
                    let page_size = Math.max(Math.ceil(data.pagination.total /data.pagination.limit),1);
                    let pageNo = data.pagination.page_no ? data.pagination.page_no : 1;
                    let pagi ='page_no='+ (pageNo > page_size ? 1 : pageNo) + '&limit=' + (data.pagination.limit ? data.pagination.limit : 10);
                    let sort = data.sort? ((data.sort.field? ('&order_by='+ data.sort.field) : '') + (data.sort.sort? ('&order=' + data.sort.sort) : '')) : '';
                    ajaxParams.data = pagi + sort + Plugin.getOption('data.source.read.params');
                    // console.log(ajaxParams.data);
                    // // Mostafiz Version End
				}
			...
			paging: function(meta, callback) {
				var pg = {
					...
					init: function(meta) {
						...
						pg.meta.pages = Math.max(Math.ceil(pg.meta.total / pg.meta.limit), 1);

						// current page must be not over than total pages
						if (pg.meta.page_no > pg.meta.pages) pg.meta.page_no = pg.meta.pages;
						...
						pg.meta.page_no = Math.max(pg.meta.page_no || 1, pg.meta.page_no);
					...
					populate: function() {
						...
						var end = Math.ceil(pg.meta.page_no / pagesNumber) * pagesNumber;
					...
					updatePerpage: function(e) {
						...
						if (e.originalEvent) {
							pg.meta.limit = parseInt($(this).val());
						}

						$(pg.pager).
							find('select.m-datatable__pager-size').
							val(pg.meta.limit).
							attr('data-selected', pg.meta.limit);
					...
					addPaginateEvent: function(e) {
						...
						$(datatable).
							...
							on(pg.paginateEvent, function(e, meta) {
								...
								$(pagerNumber).
									find('.m-datatable__pager-link-number[data-page="' + meta.page_no + '"]').
									addClass('m-datatable__pager-link--active');

								// set next and previous link page number
								$(pagerNumber).
									find('.m-datatable__pager-link--prev').
									attr('data-page', Math.max(meta.page_no - 1, 1));
								$(pagerNumber).
									find('.m-datatable__pager-link--next').
									attr('data-page', Math.min(meta.page_no + 1, meta.pages));

								// current page input value sync
								$(pg.pager).each(function() {
									$(this).
										find('.m-pager-input[type="text"]').
										prop('value', meta.page_no);
								});
					...
					updateInfo: function() {
						var start = Math.max(pg.meta.limit * (pg.meta.page_no - 1) + 1, 1);
                        var end = Math.min(start + parseInt(pg.meta.limit) - 1, pg.meta.total);
						...
						$(pg.pager).
							...
							html(Plugin.dataPlaceholder(
								Plugin.getOption('translate.toolbar.pagination.items.info'), {
									start: start,
									end: pg.meta.limit === -1 ? pg.meta.total : end,
									pageSize: pg.meta.limit === -1 ||
									pg.meta.limit >= pg.meta.total
										? pg.meta.total
										: pg.meta.limit,
									total: pg.meta.total,
								}));
					...
					paginationUpdate: function() {
						...
						if (pg.meta.page_no === pg.meta.pages) {
							$(pagerNext).
								attr('disabled', true).
								addClass('m-datatable__pager-link--disabled');
							$(pagerLast).
			...
			setDataSourceParam: function(param, value) {
				datatable.API.params = $.extend({}, {
					pagination: {page_no: 1, limit: Plugin.getOption('data.pageSize')},
