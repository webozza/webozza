...
export class Helpers {
	...
	static removeCSS(search){
		let styleSheets:any = $("head link");
		for (let i of styleSheets) {
			if (i.href && i.href.indexOf(search)>-1) {
	...
	static removeScript(script){
		let s:any  = $("body script");
		for (let i of s) {
			if (i.src && i.src.indexOf(script)>-1) {
	...
	static removeScriptFromHead(tag, id){
		let s:any  = $(tag + ' script');
		for (let i of s) {
			if (i.id && i.id==id) {
				document.head.removeChild ( i );
			}
	...
	static removeNoScriptFromHead(tag){
		let s:any  = $(tag + ' noscript');
		for (let i of s) {
			if (i.tagName == 'NOSCRIPT') {
				document.head.removeChild ( i );
			}
		}
	}
